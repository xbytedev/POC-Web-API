                            ...
                            <div class="card mt-4">
                                <div class="card-body p-4">
                                    <div class="text-center mt-2">
                                        <h5 class="text-primary">OTP Verify</h5>
                                    </div>
                                    <div class="p-2 mt-4">
                                        <form action="{{url('check_otp')}}" method="post">
                                            @csrf
                                            <div class="mb-3">
                                                <label for="username" class="form-label">OTP</label>
                                                <input type="number" name="otp" max="10000" class="form-control" id="username" placeholder="Enetr OTP"/>
                                            </div>
                                            <input type="hidden" name="user_id" value="{{request()->segment(count(request()->segments()))}}">

                                            <div class="mt-4">
                                                <button class="btn theme-btn w-100 text-white" type="submit">Verify</button>
                                            </div>
                                        </form>
                                    </div>
                                </div>
